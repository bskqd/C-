<template>
  <b-card v-if="Object.keys(sailorDocument).length" header-tag="header">
    <template #header>
      <div class="flex-row-sb">
        <div class="text-uppercase text-left">
          {{ $t(`${Object.keys(sailorDocument.behavior)[0]}-${type}`, { number: sailorDocument.number_document }) }}
        ...
        <div class="documentIconControl">
          ...
          />
          <unicon
            @click="back('education-documents')"
            name="multiply"
            fill="#42627e"
            height="25px"
            width="25px"
    ...
    <VerificationSteps
      ...
      :sailorDocument="sailorDocument"
      getFunctionName="getEducationDocs"/>
...
<script src="./SailorEducationDocument.js" />
