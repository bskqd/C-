<template>
  <b-card v-if="Object.keys(sailorDocument).length" header-tag="header">
    <template #header>
      <div class="flex-row-sb">
        <div class="text-uppercase text-left">
          {{ $t(`${Object.keys(sailorDocument.behavior)[0]}-${type}`, { page: sailorDocument.number_page_book }) }}
        ...
        <div class="documentIconControl">
          ...
          />
          <unicon
            v-if="sailorDocument.status_line.id === 34 && checkAccess(type, 'verificationSteps', sailorDocument)"
          ...
          />
          <unicon
            @click="back('experience-records-info')"
            name="multiply"
            fill="#42627e"
            height="25px"
            width="25px"
    ...
    <VerificationSteps
      ...
      :sailorDocument="sailorDocument"
      getFunctionName="getRecordBookLineEntry"/>
...
<script src="./SailorRecordBookLineDocument.js" />
